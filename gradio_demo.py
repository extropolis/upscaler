...
def supir_process(inputs: List[MediaData], a_prompt, n_prompt, num_samples,
    ...
    for image_data in inputs:
        ...
        print("Image_data")
        print(type(image_data.outputs[0]))
        print(image_data.outputs[0])
        image_path = image_data.outputs[0]
    ...
    # if len(output_data) > 0 and len(output_data[0].outputs) > 0:
    #     result_image = output_data[0].outputs[0]
    ...
    #     return result_image
    # else:
    #     return f"Image Upscaling Completed: processed {total_images} images in {main_end_time - main_begin_time:.2f} seconds #{unique_counter}"
